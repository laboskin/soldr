...
\app\assets\AppIndexAsset::register($this);
$this->title = 'Главная - soldr';
?>
<?php
function debug($arr)
{
...
<div class="content">
    <div class="content-side <?= (Yii::$app->user->isGuest)?'content-side-guest':''?>">
        <?php if(Yii::$app->user->isGuest): ?>
        <div class="login-block login-block-visible">
                ...
                <div class="login-block-icon">
                    <svg viewBox="-42 0 512 512.001" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="m426.128906 393.703125c-.691406-9.976563-2.089844-20.859375-4.148437-32.351563-2.078125-11.578124-4.753907-22.523437-7.957031-32.527343-3.3125-10.339844-7.808594-20.550781-13.375-30.335938-5.769532-10.15625-12.550782-19-20.160157-26.277343-7.957031-7.613282-17.699219-13.734376-28.964843-18.199219-11.226563-4.441407-23.667969-6.691407-36.976563-6.691407-5.226563
                                ...
                                20.238281 3.75 29.144531v.007812c.636719 9.058594.957031 18.527344.960937 28.148438-.003906 18.449219-5.214844 32.378906-15.9375 42.582031zm0 0"/></svg>
                </div>
                <div class="login-block-text">
                    Вход
                </div>
            </div>
            <?php if ($loginStatus==\app\models\User::LOGIN_STATUS_OK): ?>
            <?php $formLogin = ActiveForm::begin(); ?>
            <?= $formLogin->field($loginModel, 'email')->input('email', ['placeholder'=>'Email'])->label(false) ?>
            <?= $formLogin->field($loginModel, 'password')->passwordInput(['placeholder'=>'Пароль'])->label(false) ?>
            <div class="login-block-checks">
                <?= $formLogin->field($loginModel, 'rememberMe')->checkbox()?>
                <?= Html::a('Забыли пароль?', ['user/restore']) ?>
            </div>
            <div class="form-group">
                <?= Html::submitButton('Войти', ['class' => 'loginButton']) ?>
            </div>
            <div class="show-signup">Регистрация</div>
            <?php ActiveForm::end(); ?>
            <?php endif; ?>
            <?php if($loginStatus==\app\models\User::LOGIN_STATUS_MAIL_SENT): ?>
            <div class="mail-sent">
                <p>
            ...
            <?php endif; ?>
            <?php if($loginStatus==\app\models\User::LOGIN_STATUS_NEED_CONFIRM): ?>
            <div class="need-confirm">
                <p>Ваша учётная запись на подтверждена. При регистрации мы отправили письмо на ваш почтовый ящик.</p>
                <p>Если письмо не пришло в течение 15 минут, проверьте папку «Спам».</p>
                <p>Если же письма нет и в папке «Спам», попробуйте подписаться ещё раз. Возможно, вы ошиблись при вводе адреса.</p>
            </div>
            <?php endif; ?>
        </div>
        <div class="signup-block">
            <div class="signup-block-title">
                <div class="signup-block-icon">
                    <svg viewBox="-42 0 512 512.001" xmlns="http://www.w3.org/2000/svg">
                        <path d="m210.351562 246.632812c33.882813 0 63.21875-12.152343 87.195313-36.128906 23.96875-23.972656 36.125-53.304687 36.125-87.191406
                                0-33.875-12.152344-63.210938-36.128906-87.191406-23.976563-23.96875-53.3125-36.121094-87.191407-36.121094-33.886718 0-63.21875 12.152344-87.191406
                                36.125s-36.128906 53.308594-36.128906 87.1875c0 33.886719 12.15625 63.222656 36.128906 87.195312 23.980469 23.96875 53.316406 36.125 87.191406
                ...
                <div class="signup-block-text">
                    Регистрация
                </div>
            </div>
            <?php $formSignup = ActiveForm::begin(['class'=>'form-horizontal', 'enableAjaxValidation' => true, 'enableClientValidation' => false, ]); ?>
            <?= $formSignup->field($signupModel, 'email')->input('email', ['placeholder'=>'Email'])->label(false) ?>
            <?= $formSignup->field($signupModel, 'password')->passwordInput(['placeholder'=>'Пароль'])->label(false) ?>
            <?= $formSignup->field($signupModel, 'phone')->widget(MaskedInput::classname(), [
            ...
            <?= $formSignup->field($signupModel, 'name')->textInput(['placeholder'=>'Имя'])->label(false) ?>
            <div class="form-group">
                <?= Html::submitButton('Зарегистрироваться', ['class' => 'signupButton']) ?>
            </div>
        ...
        <?php endif; ?>
        <?php if(!Yii::$app->user->isGuest): ?>
        <?php $user = \app\models\User::findOne(Yii::$app->user->getId()) ?>
    ...
    <div class="content-main">
        <div class="content-main-nav">
            <div class="title">
                <h1 class="title-name">
                    Рекоммендации для вас<?= ($user)?', '.$user->name:'' ?>
                </h1>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.show-signup').click(function(){
            $('.login-block').removeClass('login-block-visible');
            $('.signup-block').addClass('signup-block-visible');
        });
        $('.show-login').click(function(){
            $('.login-block').addClass('login-block-visible');
            $('.signup-block').removeClass('signup-block-visible');
        });
        if(!userMobile)
        {
            $(".item").mouseover(
